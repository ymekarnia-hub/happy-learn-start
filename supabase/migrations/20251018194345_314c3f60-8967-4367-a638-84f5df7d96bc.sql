-- Corriger le trigger pour calculer le crédit du parrain sur le montant original (avant réduction)
    ...
    IF NOT EXISTS (
      ...
      IF v_referrer_id IS NOT NULL THEN
        ...
        IF v_plan_billing_period = 'annual' THEN
          -- Récupérer le montant original depuis l'historique des réductions
          SELECT original_price INTO v_original_amount
          FROM referral_discount_history
          WHERE referee_id = v_referee_id
          AND subscription_payment_id = NEW.id
          LIMIT 1;
          
          -- Si pas trouvé dans l'historique, utiliser le montant payé
          IF v_original_amount IS NULL THEN
            v_original_amount := NEW.amount_paid;
          END IF;
          
          -- Pour une formule annuelle : le crédit est 5% du montant ORIGINAL (avant réduction)
          v_credit_amount := ROUND(v_original_amount * (v_credit_percentage / 100), 2);
