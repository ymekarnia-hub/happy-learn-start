-- Fonction corrigée pour activer le parrainage au premier paiement (filleul)
-- La réduction de 5% est appliquée sur le prix TOTAL de la formule scolaire (10 mois)
  ...
  IF NOT EXISTS (
    ...
    IF v_pending_referral_code IS NOT NULL THEN
      ...
      IF v_referrer_id IS NOT NULL THEN
        ...
        -- Appliquer la réduction uniquement pour les formules annuelles (scolaires)
        IF v_plan_billing_period = 'annual' THEN
          ...
          -- Calculer la réduction de 5% sur le prix TOTAL
          v_discount_amount := ROUND(v_base_price_total * (v_discount_percentage / 100), 2);
          v_final_amount := v_base_price_total - v_discount_amount;
          ...
          INSERT INTO referral_discount_history (
            ...
            v_discount_amount,
            v_final_amount,
            'Réduction de 5% appliquée au filleul sur le montant total de la formule scolaire (' || v_duration_months || ' mois)'
          );
          
          UPDATE subscription_payments
          SET 
            amount_paid = v_final_amount,
            notes = COALESCE(notes || ' | ', '') || 'Réduction parrainage filleul: -' || v_discount_amount || ' DA (5% sur ' || v_base_price_total || ' DA)'
          WHERE id = NEW.id;
        END IF;
...
-- Fonction corrigée pour ajouter le crédit au parrain
-- Le crédit de 5% est calculé sur le prix TOTAL de la formule scolaire (10 mois)
    ...
    IF NOT EXISTS (
      ...
      IF v_referrer_id IS NOT NULL THEN
        ...
        IF v_plan_billing_period = 'annual' THEN
          ...
          -- Générer le code promo avec le crédit accumulé
          DECLARE
            v_promo_code text;
